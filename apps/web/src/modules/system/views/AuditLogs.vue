  ...
  <main class="audit-page">
    ...
    <section class="table-panel">
      ...
      <template v-else-if="logs.length">
        <el-table :data="logs" class="audit-table" stripe>          <el-table-column label="时间" min-width="178">
    ...
    <el-drawer v-model="drawerVisible" title="日志详情" :size="'min(560px, 92vw)'">
      <template v-if="selectedLog">
        ...
        <dl class="detail-grid">
          ...
          <div><dt>资源</dt><dd>{{ entityTypeLabel(selectedLog.entityType) }} / {{ selectedLog.entityId || '--' }}</dd></div>
...
@media (max-width: 640px) { .audit-stats { grid-template-columns:1fr 1fr; }.stat-card:last-child { grid-column:1/-1; }.filter-row > *,.filter-row > :first-child { min-width:100%; }.audit-table,.pagination-wrap { display:none; }.mobile-events { display:block; }.event-item { width:100%; display:flex; align-items:center; gap:10px; padding:14px 10px; border:0; border-bottom:1px solid var(--border); background:transparent; color:inherit; text-align:left; cursor:pointer; }.event-dot { width:8px; height:8px; border-radius:50%; background:var(--accent); box-shadow:0 0 0 4px color-mix(in oklch, var(--accent) 15%, transparent); }.event-content { display:flex; flex:1; flex-direction:column; gap:4px; font-size:.75rem; }.event-content span { opacity:.55; }.detail-grid { grid-template-columns:1fr; } }
